...
export default function DepartamentosPage() {
  ...
  const openModalForEdit = (departamento) => {
    setEditingDepartamento(departamento);
    setNombre(departamento.nombre);
    setNomenclatura(departamento.nomenclatura);
    setModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    const url = "/api/dashboard/departamentos";

    const payload = editingDepartamento
      ? { pkid: editingDepartamento.pkid, nombre, nomenclatura }
      : { nombre, nomenclatura };
  ...
  const visibleDepartamentos = filteredDepartamentos.slice(startIndex, startIndex + departamentosPerPage);
      ...
      <div className="bg-white p-6 rounded-2xl">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full table-auto text-gray-800">
            ...
            <tbody>
              {visibleDepartamentos.map((departamento, index) => (
                <tr key={index} className="border-b">
                  <td className="p-3">{departamento.pkid}</td>
                  <td className="p-3">{departamento.nombre}</td>
                  <td className="p-3">{departamento.nomenclatura}</td>
                  <td className="p-3 text-right">
                    <button
                      onClick={() => openModalForEdit(departamento)}
